...
fn two_args_without_path_errors_and_moves_nothing() {
    ...
    assert!(
        !out.status.success(),
        "expected failure when no explicit path given"
    );
    let stderr = String::from_utf8_lossy(&out.stderr);
    // Program prints simplified Display message; ensure strict no-auto-pick semantics.
    assert!(
        stderr.contains("No file found under base") || stderr.contains("none_found"),
        "stderr did not contain expected none_found message: {stderr}"
    );

    // Ensure original file still in download and not moved
    assert!(untouched.exists(), "file should remain in download base");
    assert!(
        !completed.join("should_not_move.bin").exists(),
